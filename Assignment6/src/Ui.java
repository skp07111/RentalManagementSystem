...
public class Ui {
	public static void main(String[] args) throws Exception {
		...
		try {
			dis = new DataInputStream(new FileInputStream("rental.dat"));
			act = new Manager(dis);
			...
			while (true) {
				...
				// 대답이 예스일 경우
					...
					System.out.println("새로운 파일에 작성합니다.");
					act = new Manager();
					break;
				}
				// 잘못 입력할 경우
		...
		while(true)
		{
			...
			case 9: // 파일 저장하기
				try {
					dos = new DataOutputStream(new FileOutputStream("rental.dat")); // DataOutputStream 객체 dos 생성
					act.writeFile(dos); // Manager 클래스의 writeFile 메소드 콜
